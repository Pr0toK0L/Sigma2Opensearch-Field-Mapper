...
class SigmaConverter:
    ...
    def setup_upload_tab(self):
        ...
        ttk.Radiobutton(file_frame, text="Single File", variable=self.upload_mode_var,
                        value="single", command=self.toggle_upload_mode, style="TRadiobutton").grid(row=0, column=1, padx=PADX_DEFAULT, pady=PADY_DEFAULT)
        ttk.Radiobutton(file_frame, text="Folder", variable=self.upload_mode_var,
                        value="folder", command=self.toggle_upload_mode, style="TRadiobutton").grid(row=0, column=2, padx=PADX_DEFAULT, pady=PADY_DEFAULT)
    ...
    def _upload_to_opensearch(self):
        """Handle the upload process to OpenSearch (placeholder)."""
        try:
            self.root.after(0, lambda: self.upload_status_var.set("Upload completed."))
            self.root.after(0, lambda: self.results_text.insert(tk.END, "Upload successful\n"))
    ...
    def convert_sigma_to_dql(self, input_file, output_file, field_mapping_file):
        """Convert Sigma rule to OpenSearch DQL query and save to file."""
        try:
            field_mappings = self.load_field_mappings(field_mapping_file)
...
"""
    ...
    def convert_single_file(self):
        ...
        if self.single_output_option.get() == "file":
            ...
            extension = ".yml" if self.conversion_type_var.get() == "winlogbeat" else ".txt"
            output_file = os.path.join(output_folder, f"{file_name}{extension}")
        ...
        if self.conversion_type_var.get() == "winlogbeat":
            ...
            success = self.convert_sigma_to_dql(input_file, output_file, mapping_file)
    ...
    def convert_multiple_files(self):
        ...
        for file_path in file_list:
            ...
            extension = ".yml" if self.conversion_type_var.get() == "winlogbeat" else ".txt"
            output_file = os.path.join(output_folder, f"{name_without_ext}{extension}")

            success = False
            if self.conversion_type_var.get() == "winlogbeat":
                success = self.update_sigma_rule(file_path, output_file, mapping_file)
            else:
                success = self.convert_sigma_to_dql(file_path, output_file, mapping_file)
    ...
    def setup_converter_tab(self):
        ...
        ttk.Radiobutton(conversion_frame, text="Winlogbeat", variable=self.conversion_type_var,
                        value="winlogbeat", style="TRadiobutton").grid(row=0, column=0, padx=20, pady=10)
        ttk.Radiobutton(conversion_frame, text="OpenSearch DQL", variable=self.conversion_type_var,
                        value="dql", style="TRadiobutton").grid(row=0, column=1, padx=20, pady=10)
